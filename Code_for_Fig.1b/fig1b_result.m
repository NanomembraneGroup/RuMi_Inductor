...
n_values = 1:10; % Number of turns (n)
l_tot_1_values = 4 * (n_values.^2+n_values) * w; % Total length for L3, M3, L4, M4
l_tot_2_values = 2 * pi * R * n_values; % Total length for L5, M5, L6, M6
...
for idx = 1:length(n_values)
    ...
    for i = 2:n
        for j = 1:i-1
            ...
            a1 = (j - 1) * w;
            a2 = j * w;
            M_4_mag = M_4_mag + 1e9 * 2 * Zp_t2(mu_r, h, r1, r2, a1, a2, h);
        end
    end
    M_4 = M_3 + M_4_mag;
    ...
    % Compute L5 and M5
    Bz_avg_self = average_magnetic_field_Z_range(R, -w / 2, w / 2, R, h, w, 50, 50);
    L_5 = 1e9 * n * Bz_avg_self * pi * R^2;
    for j = 2:n
        for k = 1:j-1 
            Bz_avg = average_magnetic_field_Z_range(R + (k - 1) * 2 * h, -w / 2, w / 2, R + (j - 1) * 2 * h, h, w, 50, 50);
            M_5 = M_5 + 2 * 1e9 * Bz_avg *  pi * (R + (k - 1)*h)^2;
        end
    end
    ...
    for j = 1:n
        for k = 1:j
            Bz_avg = average_magnetic_field_on_line(R - (k - j) * 2 * h, 0, -w / 2, w / 2, R + (j - 1) * 2 * h, h, w, 50, 50, 50);
            L_6_mag = L_6_mag + 1e9 * mu_r * Bz_avg * 2 * pi * (R - (k - j) * 2 * h) * h;
        end
    end
    L_6 = L_5 + L_6_mag;
    ...
    for j = 1:n
        for k = 1:j
            for m = 1:k
                Bz_avg = average_magnetic_field_on_line(R - (m - j) * 2 * h, 0, -w / 2, w / 2, R + (j - 1) * 2 * h, h, w, 50, 50, 50);
                M_6_mag = M_6_mag + 1e9 * mu_r * Bz_avg * 2 * pi * (R - (m - j) * 2 * h) * h;
            end
        end
    end
    M_6 = M_5 + M_6_mag;
